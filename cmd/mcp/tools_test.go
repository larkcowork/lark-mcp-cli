...
func TestReadToolsExposeJq(t *testing.T) {
	// NOTE: lark_base_search is intentionally excluded. The `base +record-search`
	// shortcut defaults to markdown and rejects --jq ("--jq and --format markdown
	// are mutually exclusive") even when --format is not set explicitly, so jq is
	// unusable there. lark_base_search forces --format json and narrows output via
	// select_fields/limit instead. See toolBaseSearch.
