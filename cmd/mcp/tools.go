...
func allTools() []tool {
	return []tool{
		...
		toolOKRCycleList(),
		toolTaskComplete(),
		toolBaseRecordUpsert(),
		toolWikiNodeCreate(),
		toolCalendarFreebusy(),
		toolGenericAPI(),
	}
}
...
func toolIMSearch() tool {
  ...
  "properties": {
    ...
    "jq":        {"type": "string", "description": "Optional jq expression to project/filter the JSON output (e.g. '.data.messages[] | {id, chat_id, body}'). Cuts token cost of large result sets. Mutually exclusive with --format."},
...
func toolCalendarAgenda() tool {
  ...
  "properties": {
    ...
    "jq":          {"type": "string", "description": "Optional jq projection (e.g. '.data[] | {summary, start_time, end_time}'). Reduces token cost vs full event payload."},
    "as":          {"type": "string", "enum": ["user", "bot"]}
  }
}`,
		Build: func(args map[string]interface{}) ([]string, error) {
...
func toolDocSearch() tool {
  ...
  "properties": {
    ...
    "jq":        {"type": "string", "description": "Optional jq projection (e.g. '.data.results[] | {title, token, url}'). Cuts token cost vs full search response."},
    "as":        {"type": "string", "enum": ["user", "bot"]}
  },
  "required": ["query"]
}`,
...
func toolDocFetch() tool {
	return tool{
		Name:        "lark_doc_fetch",
		Description: "Fetch a Lark Doc's full contents as markdown. Always uses v2 API. Use when you need to read, summarize, translate, or quote from a specific doc whose token/URL you already have. For large docs (>20 pages), use the jq arg to project a slice (e.g. '.data.content[:5000]' for first 5k chars, or '.data.title' for just metadata) to control token cost. To find docs first when you don't know the token, use lark_doc_search.",
		Schema: `{
  "type": "object",
  "properties": {
    "doc": {"type": "string", "description": "Document URL or token (the id segment in the doc URL)."},
    "jq":  {"type": "string", "description": "Optional jq projection. A full doc can be 25k+ tokens; use jq to slice (e.g. '.data.content[:5000]' for first 5k chars, or '.data.title' for just metadata)."},
    "as":  {"type": "string", "enum": ["user", "bot"]}
  },
  "required": ["doc"]
}`,
...
func toolBaseSearch() tool {
	return tool{
		...
		Description: "Search records in a Lark Base (Bitable) table by keyword. " +
			...
			"or check whether a record exists. " +
				"IMPORTANT: the Bitable API REQUIRES search_fields — say which field(s) to match the keyword in " +
				"(e.g. search_fields=\"Name\"). If you don't know field names, first call " +
				"lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields. " +
				"Returns JSON; narrow output with select_fields + limit (this tool does not support jq). " +
				"For full control pass search_json. For counts/sums use lark_api base data-query.",
		Schema: `{
  "type": "object",
  "properties": {
    ...
    "query":          {"type": "string", "description": "Search keyword."},
    "search_fields":  {"type": "string", "description": "REQUIRED with query. Comma-separated field name(s)/id(s) to match the keyword in (the Bitable API mandates this). Discover names via lark_api GET .../tables/{table}/fields."},
    "select_fields":  {"type": "string", "description": "Optional. Comma-separated field names/ids to return (narrows the payload instead of jq)."},
    "view_id":        {"type": "string", "description": "Optional. Scope to a specific view."},
    "limit":          {"type": "integer", "description": "Optional. Records per page (1-200, default 10) — use a small limit to bound payload size."},
    "search_json":    {"type": "object", "description": "Advanced: pass the full search JSON object directly (must itself include search_fields). Overrides query/search_fields/etc when present."},
    "as":             {"type": "string", "enum": ["user", "bot"]}
  },
  "required": ["base_token", "table_id"]
		...
		Build: func(args map[string]interface{}) ([]string, error) {
			...
			if raw, ok := args["search_json"]; ok && raw != nil {
				...
				sf, _ := argString(args, "search_fields")
				if sf == "" {
					return nil, fmt.Errorf("search_fields is required for a keyword search — the Bitable API needs which field(s) to match the keyword in (e.g. \"Name\" or \"Name,Notes\"). Run lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields to discover field names, or pass search_json for a full request body")
				}
				payload := map[string]interface{}{"keyword": q, "search_fields": strings.Split(sf, ",")}
			...
			// Always request JSON output: base +record-search defaults to
			// markdown, which (a) is verbose and (b) makes --jq error out
			// ("--jq and --format markdown are mutually exclusive"). JSON is
			// the right shape for an MCP consumer; project client-side or via
			// the limit/select_fields args rather than --jq for this tool.
...
func toolContactSearch() tool {
  ...
  "properties": {
    ...
    "jq":        {"type": "string", "description": "Optional jq projection (e.g. '.data.users[] | {open_id, name, email, department}'). Avoids fetching full directory metadata into context."},
    "as":        {"type": "string", "enum": ["user", "bot"]}
  },
  "anyOf": [
    {"required": ["query"]},
...
func toolTaskMy() tool {
  ...
  "properties": {
    ...
    "jq":         {"type": "string", "description": "Optional jq projection (e.g. '.data.items[] | {id, summary, due, completed}'). Reduces token cost for large task lists."},
    "as":         {"type": "string", "enum": ["user", "bot"]}
  }
}`,
		Build: func(args map[string]interface{}) ([]string, error) {
...
func toolSheetsRead() tool {
  ...
  "properties": {
    ...
    "jq":                 {"type": "string", "description": "Optional jq projection (e.g. '.data.valueRange.values[][0:3]' to keep only first 3 columns). Cuts token cost for wide sheets."},
    "as":                 {"type": "string", "enum": ["user", "bot"]}
  },
  "anyOf": [
...
func toolVCSearch() tool {
  ...
  "properties": {
    ...
    "jq":              {"type": "string", "description": "Optional jq projection (e.g. '.data.items[] | {id, display_info}'). Each item has id, display_info, and meta_data.{app_link,avatar,description}. Reduces token cost vs full meeting payload."},
    "as":              {"type": "string", "enum": ["user", "bot"]}
  }
}`,
		Build: func(args map[string]interface{}) ([]string, error) {
...
func toolMinutesSearch() tool {
  ...
  "properties": {
    ...
    "page_size":       {"type": "integer", "description": "Page size, 1-30 (default 15)."},
    "jq":              {"type": "string", "description": "Optional jq projection (e.g. '.data.items[] | {token, topic, owner_id, create_time}'). Skips full transcript download into context."},
    "as":              {"type": "string", "enum": ["user", "bot"]}
  }
}`,
		Build: func(args map[string]interface{}) ([]string, error) {
...
func toolOKRCycleList() tool {
  ...
  "properties": {
    ...
    "jq":           {"type": "string", "description": "Optional jq projection (e.g. '.data.cycles[] | {id, name, status}'). Reduces token cost when listing many cycles."},
    "as":           {"type": "string", "enum": ["user", "bot"]}
  },
  "required": ["user_id"]
}`,
