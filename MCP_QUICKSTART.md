...
Install the [lark-cowork-plugins](https://github.com/larkcowork/lark-cowork-plugins)
marketplace (`lark-cli-dev` plugin) for the `/mcp-add <name>` slash command and the
`lark-cli-mcp` skill — together they walk through schema, builder, build, and verify.
See [`cmd/mcp/README.md`](cmd/mcp/README.md) for the full architecture and the canonical recipe.
